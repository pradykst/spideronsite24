...
contract SimpleMintContract is ERC721, Ownable{
    ...
    constructor() payable ERC721('Simple Mint','SM') Ownable(msg.sender){
        maxSupply = 2;
    }
    ...
    function verifyOwnership(uint256 tokenId) external view returns (bool) {
        return (tokenId <= totalSupply) && ownerOf(tokenId) == msg.sender;
    }

    function getOwnedTokenId() public view returns (uint256 id) {
